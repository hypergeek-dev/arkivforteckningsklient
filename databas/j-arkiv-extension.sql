-- ARKIVFÖRTECKNING EXTENSION --
-- Extends existing IHPv tables with archival metadata fields.
-- Safe to re-run: all statements use ADD COLUMN IF NOT EXISTS or CREATE TABLE IF NOT EXISTS.
-- Follows existing schema conventions (search_path = ihp, snake_case, CHARACTER VARYING).
--
-- Mapping:
--   operationalareatypenode  → Arkivbildare
--   processgrouptypenode     → Arkiv
--   processtypenode          → Serie
--   issuetypenode            → Underserie
--   documenttypenode         → Volym / Handlingstyp
--   rule                     → Gallrings- och bevaranderegel
-- -----------------------------------------------------------------------
...
COMMENT ON COLUMN processgrouptypenode.forvaringsplats     IS 'Förvaringsplats för arkivet (fysisk plats eller digitalt system)';
...
COMMENT ON COLUMN processtypenode.serie_rubrik    IS 'Beskrivande rubrik för serien (utöver name)';
...
COMMENT ON COLUMN rule.ra_fs_referens IS 'Hänvisning till RA-FS eller annan författning (t.ex. RA-FS 2019:1 § 4)';
...
-- -----------------------------------------------------------------------
-- arkivforeteckning_metadata (Arkivförteckning – toppnivåmetadata)
-- One row per classificationstructuretypenode used as an Arkivförteckning.
-- -----------------------------------------------------------------------
...
COMMENT ON TABLE  arkivforeteckning_metadata                IS 'Toppnivåmetadata kopplad till en Arkivförteckning (classificationstructuretypenode)';
...
-- END ARKIVFÖRTECKNING EXTENSION --
